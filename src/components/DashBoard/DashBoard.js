  ...
  return (
    <Box sx={{
      ...
      <Box>
        ...
        <Button variant="outlined" onClick={() => changeCountValue('minus')} sx={{}}>-</Button>
        <Button variant="contained" sx={{ textDecoration: 'none' }}>Reset</Button>
        <Button variant="outlined" onClick={() => changeCountValue('plus')} sx={{}}>+</Button>
      </Box>
    </Box>
  )
}
...
const DashBoard = (props) => {
  ...
  const onSubmit = () => {
    if (!user?.name || !user?.address || !user?.phone || !user?.email) {
      alert('please fill require feilds')
      return;
    }
  ...
  return (
    <div>
      <nav className="navbar navbar-expand-lg navbar-dark bg-dark static-top">
        <h1 style={{ color: '#fff' }}>HELLO {localUname}</h1>
      ...
      <Grid container>
        ...
        <Grid lg={12}>
          ...
          <Box textAlign={'center'} my={2}>
            ...
            }} sx={{ ml: 2 }}>trigger animation</Button>
          </Box>
        </Grid>
      </Grid>
      ...
      {logout && (
        <SweetAlert
          ...
          title={localStorage.getItem('user') == JSON.stringify(user) && localStorage.getItem('text') == JSON.stringify(text) ? "Are you sure?" : "Do you want to save this changes then logout"}
